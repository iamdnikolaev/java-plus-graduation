...
public class AggregationStarter {
    ...
    public void start() {
        ...
        Runtime.getRuntime().addShutdownHook(new Thread(() -> {
            log.info("Возник хук на завершение JVM. Прерываем работу консьюмера.");
            consumer.wakeup();
        }));
        ...
        try {
            ...
            while (true) {
                ...
                for (ConsumerRecord<String, UserActionAvro> record : records) {
                    log.info("== userActionAvro получен {}", record);
                    ...
                    if (eventSimilarityAvroOptionalList.isPresent()) {
                        ...
                        for (EventSimilarityAvro eventSimilarityAvro : eventSimilarityAvroList) {
                            ...
                            log.info("В топик {} отправляем A = {} и B = {} eventSimilarityAvro {}",
            ...
            log.info("Ошибка в действиях пользователя", e);
        } finally {
            try (consumer; producer) {
                producer.flush();
                consumer.commitSync(currentOffsets);
            } finally {
                log.info("Aggregator consumer закрывается");
                consumer.close();
                log.info("Aggregator producer закрывается");
                producer.close();
            }
        }
    }
    ...
    private void manageOffsets(ConsumerRecord<String, UserActionAvro> record, int count,
        ...
        if (count % 10 == 0) {
            consumer.commitAsync(currentOffsets, (offsets, exception) -> {
                if (exception != null) {
                    log.warn("Ошибка в фиксации смещений: {}", offsets, exception);
                }
            });
        }
